...
public class CircularEventCellView extends BaseCellView {
    ...
    private void init(AttributeSet attrs){
        ...
        try{
            radius = (int)a.getDimension(R.styleable.CircularEventCellView_event_radius,2);
            padding= (int)a.getDimension(R.styleable.CircularEventCellView_event_circle_padding,0);
        }finally {
            a.recycle();
        }
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if((stateSet==null || stateSet.isEmpty()
          ...
          //  p.getTextBounds("31", 0, 1, rect); // measuring using fake text
            p.getTextBounds("25", 0, 1, rect); // measuring using fake text
